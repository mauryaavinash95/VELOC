...
bool veloc_client_t::checkpoint_mem(int mode, std::set<int> &ids) {
	...
	for (auto &e : ckpt_regions) {
        ...
        cudaPointerGetAttributes (&attributes, e.second.first);
        // attributes.type can be one of cudaMemoryTypeUnregistered (unpinned), 
        // cudaMemoryTypeHost (pinned), cudaMemoryTypeDevice (on-GPU), or cudaMemoryTypeManaged (managed)
        if(attributes.type==cudaMemoryTypeDevice) {
            // Copy from device to host first, and then ckpt.
            char *temp;
            char *gpu_var = (char *)e.second.first;
            cudaMallocHost((void**)&temp, e.second.second);
            cudaMemcpyAsync(temp, gpu_var, e.second.second, cudaMemcpyDeviceToHost);
